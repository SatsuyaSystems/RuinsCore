...
public final class WacheHealthService {

    private static final double DEFAULT_MAX_HEALTH = 20.0D;
    private static final double WACHE_HEALTH_BONUS = 8.0D; // 4 extra Herzen

    private final JobService jobService;

    public WacheHealthService(JobService jobService) {
        this.jobService = jobService;
    }

    public void sync(Player player) {
        boolean hasWache = jobService.hasJob(player.getUniqueId(), JobType.WACHE);
        if (hasWache) {
            applyBonus(player);
            return;
        }
    ...
    private void applyBonus(Player player) {
        ...
        double targetMaxHealth = DEFAULT_MAX_HEALTH + WACHE_HEALTH_BONUS;
